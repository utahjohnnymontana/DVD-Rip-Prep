...
The script does everything possible to produce output that matches the duration of the original, but, particularly in the case of mixed videos, there is usually some small difference.  Most of the time, this difference is so small that there won't be a visible mismatch and the video can be safely remerged with the original audio and subtitle tracks.  But there are unfortunate exceptions.  If MATCHTRACKS is enabled, then the script will adjust the audio and subtitle tracks to match the output duration.  This does mean that the audio tracks must be reencoded, which is lossy, although your ears would have to be better than mine to notice a single reencode.  This setting should ensure that you always get video and audio that match up.  If you disable it (0), then you will have to make any needed adjustments manually.
